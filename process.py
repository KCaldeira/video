...
def export_metrics_to_csv(frame_count_list, metrics, filename):
    """
    Export frame count and metric data to a CSV file.
    ...
    # Ensure all metric lists are the same length as frame_count_list
    length = len(frame_count_list)
    for key, value in metrics.items():
        if len(value) != length:
            raise ValueError(f"Length mismatch for key '{key}': expected {length}, got {len(value)}")

    # Create header
    headers = ['frame_count_list'] + list(metrics.keys())

    # Zip data rows
    rows = zip(frame_count_list, *metrics.values())
